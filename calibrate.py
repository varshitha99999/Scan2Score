...
from recognize_roll import (
    TOP_CROP_FRACTION, RIGHT_CROP_FRACTION,
...
def calibrate(image_path: str):
    ...
    if blue_count < 100:
        ...
        print("   → Edit BLUE_HSV_LOWER / BLUE_HSV_UPPER in recognize_roll.py")
    elif len(boxes) == 0:
        print("⚠  WARNING: Blue ink found but no digit shapes detected.")
        print("   → Try reducing MIN_DIGIT_AREA in recognize_roll.py")
    else:
        print(f"✅  System found {len(boxes)} digit(s) in the roll number zone.")
        print("   → Ready to run:  python recognize_roll.py your_paper.jpg")
    print("─"*55)
